...
class CL:
    ...
    def add_intermediate_buffer(self, program, name, data):
        mf = cl.mem_flags
        self.buffers[program][name] = cl.Buffer(self.ctx, mf.READ_WRITE | mf.COPY_HOST_PTR, hostbuf=data)
    ...
    def read_buffer(self, program, name, size, dtype):
        result = numpy.array(range(size), dtype=dtype)
...
if __name__ == "__main__":
    cltest = CL()
    cltest.build_program("Test", ("__kernel void test(__global float* a, __global float* b, __global float* c)\n"
                                  "{\n"
                                  "    unsigned int i = get_global_id(0);\n"
                                  "    c[i] = a[i] + b[i];\n"
                                  "}\n"
                                  ))
    test_array = numpy.array(range(10), dtype=numpy.float32)
    ...
    cltest.execute_program_kernel("Test", "test", test_array.shape, "a", "b", "c")
